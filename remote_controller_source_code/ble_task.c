...
static void user_button_isr(void *handler_arg, cyhal_gpio_event_t event);
...
// source: https://github.com/Infineon/PSoC6-101-Video-Projects/blob/master/PSoC%206_101/3-3-SimpleBLECentral.cydsn/main_cm4.c
/*****************************************************************************\
 ...
 *   this is NOT a perfect parser as it only looks for the first occurrence of
...
//                }
                ...
                default:
                {
                    task_print_error("Unknown command ok 0x%X",
...
static void stack_event_handler(uint32_t event, void* event_param)
{
    ...
    switch (event)
    {
        ...
        **********************************************************************/
        ...
        {
        	// uses status LED from dev board
...
//            }
        ...
        **********************************************************************/
        ...
        case CY_BLE_EVT_GAPC_SCAN_PROGRESS_RESULT:
                    ...
                    task_print_info("Device ");
                    cy_stc_ble_gapc_adv_report_param_t  *scanProgressParam = (cy_stc_ble_gapc_adv_report_param_t  *)event_param;
                    task_print_info("BD Address = ");
                    for(unsigned int i=0;i<CY_BLE_BD_ADDR_SIZE;i++)
                        task_print_info("%02X",scanProgressParam->peerBdAddr[i]);

                    task_print_info("Length = %d ",scanProgressParam->dataLen);

                    findAdvInfo(scanProgressParam->data,scanProgressParam->dataLen);
                    if(currentAdvInfo.name != 0)
                        task_print_info("%.*s",currentAdvInfo.name_len,currentAdvInfo.name);
                    ...
                    if(currentAdvInfo.servUUID_len > 0
                        && memcmp(currentAdvInfo.serviceUUID, cy_ble_customCServ [CY_BLE_CUSTOMC_LEDS_SERVICE_INDEX].uuid,
                        currentAdvInfo.servUUID_len) == 0)
                    {
                        task_print_info("Found LED Service");
        ...
        ***********************************************************************/
        case CY_BLE_EVT_GATTC_DISCOVERY_COMPLETE:
            task_print_info("Discovery Complete" );
        break;

        case CY_BLE_EVT_GATTC_ERROR_RSP:
            task_print_info("GATTC Error Response");
        break;

        case CY_BLE_EVT_GATTC_WRITE_RSP:
            task_print_info("GATTC Write Succeeded");
        break;
...
//
//				ledData.ece_led = ECE453_LED_GREEN;
//				ledData.pwm = write_req_param->handleValPair.value.val[0] > 100 ? 100 : write_req_param->handleValPair.value.val[0];
//				xQueueSendToBack(ece453_led_data_q, &ledData, portMAX_DELAY);
//			}
...
//                  * and what the format of the data being sent is.*/
//
//				ledData.ece_led = ECE453_LED_RED;
//				ledData.pwm = write_req_param->handleValPair.value.val[0] > 100 ? 100 : write_req_param->handleValPair.value.val[0];
//				xQueueSendToBack(ece453_led_data_q, &ledData, portMAX_DELAY);
//			}
//			break;
