...
void task_ece453_led(void *param)
{
    ...
    cyhal_pwm_init(&pwm_red_obj, PIN_LED_RED, NULL);
    cyhal_pwm_set_duty_cycle(&pwm_red_obj, 100, 1000);
    ...
    cyhal_pwm_init(&pwm_green_obj, PIN_LED_GREEN, NULL);
    cyhal_pwm_set_duty_cycle(&pwm_green_obj, 100, 1000);
    ...
    for(;;)
    {
    	...
    	ece453_led_data_t led_data;
        xQueueReceive(ece453_led_data_q, &led_data, portMAX_DELAY);
        if (led_data.ece_led == ECE453_LED_RED) {
        	cyhal_pwm_set_duty_cycle(&pwm_red_obj, 100 - led_data.pwm, 1000);
        } else if (led_data.ece_led == ECE453_LED_GREEN) {
        	cyhal_pwm_set_duty_cycle(&pwm_green_obj, 100 - led_data.pwm, 1000);
        }
    }
}
