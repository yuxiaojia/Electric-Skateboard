...
void task_esc(void *param)
{
    ...
    cyhal_pwm_t pwm_esc_obj;
    cyhal_pwm_init(&pwm_esc_obj, PIN_ESC, NULL);
    cyhal_pwm_set_duty_cycle(&pwm_esc_obj, 0, 50);
    cyhal_pwm_start(&pwm_esc_obj);
    ...
    for(;;) {
    	...
    	esc_data_t esc_data;
        xQueueReceive(esc_data_q, &esc_data, portMAX_DELAY);
        task_print_info("esc = %lf\r\n", ((double)esc_data.pwm)/5+5);  //add
        cyhal_pwm_set_duty_cycle(&pwm_esc_obj, ((double)esc_data.pwm)/5+5, 50);
