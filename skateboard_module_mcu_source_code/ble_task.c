        ...
        case CY_BLE_EVT_GATT_CONNECT_IND:
       {
		...
		case CY_BLE_EVT_GATTS_WRITE_REQ:
		{
			...
			if( CY_BLE_LEDS_GREEN_CHAR_HANDLE == write_req_param->handleValPair.attrHandle)
			{
				...
				escData.pwm = write_req_param->handleValPair.value.val[0] > 100 ? 100 : write_req_param->handleValPair.value.val[0];

				xQueueSendToBack(esc_data_q, &escData, portMAX_DELAY);
			}



			Cy_BLE_GATTS_WriteRsp(write_req_param->connHandle); //!!
			//this line for doing some stuff that helps
